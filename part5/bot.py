...
reddit = praw.Reddit(client_id="0UK5bAQCU-eK_g",
                     client_secret="hX8IC8xR21ZdPUTCUn6W_eqGW9aTtA",
                     password="Interesting-Let-3843",
                     user_agent="posted by u/Interesting-Let-3843",
                     username="Interesting-Let-3843)
...
for i in range(0, len(title_list)):
    for k in range(0, len(subredit_list)):
        try:
            ...
            com = "#[Click here for mega link]({}), This video's [Source]({}) ".format(comment_list[i], url_list[i])
            time.sleep(10)
            submission.reply(com)
            print("done")
        except Exception as err:
            print("Exception for subreddit {}, {}".format(subredit_list[k], err))
        t = random.randint(390, 420)
        seconds = "Sleeping for {} seconds before proceeding".format(t)
        print(seconds)
        time.sleep(t)
